...
def time_test(noitems, algorithm):
    ...
    for i in range(30, noitems, 3):
        ...
        solution, values, accuracy = algorithm(data[0], data[1], data[2], len(data[0]))
        exec_t = (time.time()-init)
        print("Execution time: "+str(exec_t)+" seconds")
        knapsize.append(i)
        times.append(exec_t)
...
times, knapsize, accuracies = time_test(33, knapsack)  # Usage example
print(knapsize)
print(times)
